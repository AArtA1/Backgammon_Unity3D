...
public class BoardManagerV2 : MonoBehaviour
{
    ...
    private HashSet<KeyValuePair<int, int>> legalMoves = new HashSet<KeyValuePair<int, int>>();
    ...
    {
        legalMoves.Clear();
        ConcurrentBag<KeyValuePair<int, int>> movesRoll1 = new ConcurrentBag<KeyValuePair<int, int>>();
        ConcurrentBag<KeyValuePair<int, int>> movesRoll2 = new ConcurrentBag<KeyValuePair<int, int>>();
        ...
        Task task1 = Task.Run(() => {
            if (currGamePhase == turnPhase.WhiteTurn)
            {
                if (whiteCheckerPhase == gamePhase.BaringOff)
                {
                    if (!usedRoll1)
                    {
                        foreach (var move in getBarringOffMoves(roll1, roll2, true))
                        {
                            movesRoll1.Add(move);
                        }
                    }
                    if (!usedRoll2)
                    {
                        foreach (var move in getBarringOffMoves(roll2, roll1, true))
                        {
                            movesRoll2.Add(move);
                        }
                    }
                ...
                else
                {
                    if (!usedRoll1)
                    {
                        foreach (var move in getRegularMoves(roll1, roll2, true))
                        {
                            movesRoll1.Add(move);
                        }
                    }
                    if (!usedRoll2)
                    {
                        foreach (var move in getRegularMoves(roll2, roll1, true))
                        {
                            movesRoll2.Add(move);
                        }
                    }
                    ...
                    {
                        if (movesRoll1.Count == 0 && movesRoll2.Count > 1)
                        {
                            foreach (var move in refineRegularMoves(movesRoll2.ToList(), roll1, true))
                            {
                                movesRoll2.Add(move);
                            }
                        }
                        else if (movesRoll2.Count == 0 && movesRoll1.Count > 1)
                        {
                            foreach (var move in refineRegularMoves(movesRoll1.ToList(), roll2, true))
                            {
                                movesRoll1.Add(move);
                            }
                        }
            ...
            else
            {
                if (blackCheckerPhase == gamePhase.BaringOff)
                {
                    if (!usedRoll1)
                    {
                        foreach (var move in getBarringOffMoves(roll1, roll2, false))
                        {
                            movesRoll1.Add(move);
                        }
                    }
                    if (!usedRoll2)
                    {
                        foreach (var move in getBarringOffMoves(roll2, roll1, false))
                        {
                            movesRoll2.Add(move);
                        }
                    }
                ...
                else
                {
                    if (!usedRoll1)
                    {
                        foreach (var move in getRegularMoves(roll1, roll2, false))
                        {
                            movesRoll1.Add(move);
                        }
                    }
                    if (!usedRoll2)
                    {
                        foreach (var move in getRegularMoves(roll2, roll1, false))
                        {
                            movesRoll2.Add(move);
                        }
                    }
                    if (!usedRoll1 && !usedRoll2)
                    {
                        if (movesRoll1.Count == 0 && movesRoll2.Count > 1)
                        {
                            foreach (var move in refineRegularMoves(movesRoll2.ToList(), roll1, false))
                            {
                                movesRoll2.Add(move);
                            }
                        }
                        ...
                        {
                            foreach (var move in refineRegularMoves(movesRoll1.ToList(), roll2, false))
                            {
                                movesRoll1.Add(move);
                            }
                        }
    ...
    private List<KeyValuePair<int, int>> refineRegularMoves(List<KeyValuePair<int, int>> legalMoves, int unusedRoll, bool isWhite)
    {
        ...
        return refinedMoves.Count > 0 ? refinedMoves : legalMoves;
    }
